...
# Features
 ...
 - Client-agnostic (works with multiple backends)
...
```
val foperatorVersion = "TODO" // see releases page for latest version

// core modules
libraryDependencies += "net.gfxmonk" %% "foperator-core" % foperatorVersion
libraryDependencies += "net.gfxmonk" %% "foperator-testkit" % foperatorVersion % Test

// pick a backend
libraryDependencies += "net.gfxmonk" %% "foperator-backend-skuber" % foperatorVersion
libraryDependencies += "net.gfxmonk" %% "foperator-backend-kubernetes-client" % foperatorVersion
```
...
Hand-built kubernetes client bindings.

 - concurrency: akka-streams + Futures, wrapped with Task / Observable in `foperator-skuber`
 - http: akka-http
 - serialization: play / jackson

The backend class is `foperator.backend.Skuber`.

**Note**: skuber is built on akka, which liberally employs bad practices like `Await.result`.
This means you can't use skuber within a test scheduler (e.g. TestControl from cats-effect)

## [Kubernetes-Client](https://github.com/doriordan/skuber):

Autogenerated bindings from swagger API definitions.

 - concurrency: works with any cats-effect compatible IO implementation
 - http: http4s
 - serialization: circe

The backend class is `foperator.backend.KubernetesClient`.

To make implicits available, `import foperator.backend.kubernetesclient.implicits._`

## TestClient (in `foperator-testkit`)

This is a stub backend for use in tests. It's backed by a simple in-memory hash, but it's enough to
test operations on custom resources. It works with any resource type, so you can use your real
Skuber / KubernetesClient resources, or for simplicity you can also use the TestResource type.

# Sample operators

See [./sample](./sample) for a couple of sample operators using the framework.

In particular, `Models.scala` provides a good overview of the implicit values you need to make available for each backend to be able to operate on a custom resource.
...
Most of the time, an implicit will become available when you import e.g. `foperator.backend.skuber.implicits._` (for the Skuber backend). If not, it often means that there's an implicit missing somewhere in the chain.
